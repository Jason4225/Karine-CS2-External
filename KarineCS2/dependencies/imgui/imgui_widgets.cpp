...
bool ImGui::Tab(const char* label, const ImVec2& size_arg, int currentTab, int* tab)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    const ImU32 col = (held && hovered) ? ImColor(111, 155, 252, 70) : hovered ? ImColor(111, 155, 252, 50) : ImColor(111, 155, 252, 35);
    RenderNavHighlight(bb, id);

    //RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, col, 5.f);
    if (hovered || *tab == currentTab)
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(111, 155, 252, 100), 5.f);
